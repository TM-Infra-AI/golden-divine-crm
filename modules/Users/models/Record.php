...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	public function getAccessibleUsers($private="",$module = false) {
		...
		$currentUserModule = Users_Record_Model::getCurrentUserModel();
		$currentUserRole = $currentUserModule->getRole();
		$admin_check = $currentUserModule->isAdminUser();
		if ($moduleName == 'SalesOrder') {
			if($admin_check == 0){
				if($currentUserRole == 'H4'){
					$test = array('H8','H10');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;
				}elseif ($currentUserRole == 'H8') {
					$test = array('H9');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;
				}elseif($currentUserRole == 'H9'){
					$test = array('H6');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;
				}elseif($currentUserRole == 'H10'){
					$test = array('H6');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;	
				}elseif($currentUserRole == 'H6'){
					$test = array('H5');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;
				}elseif($currentUserRole == 'H7'){
					$test = array('H5');
					$finalval = $this->assigneeCustom($test);
					$accessibleUser = $finalval;
				}else{
	...
	public function assigneeCustom($roleid){
		$db = PearDatabase::getInstance();
		$string = implode("','",$roleid);

		$ass_user = "SELECT CONCAT(vtiger_users.first_name,' ', vtiger_users.last_name) as full_name, vtiger_users.id FROM vtiger_users INNER JOIN vtiger_user2role ON vtiger_users.id = vtiger_user2role.userid WHERE vtiger_user2role.roleid IN ('$string')";
		
		$ass_res = $db->pquery($ass_user, array());
		$testarr = array();

		while($ass_row = $db->fetch_row($ass_res)) {
			$testarr[$ass_row['id']] = $ass_row['full_name'];
		}
		return $testarr;
	}
}
