...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		//Custom for List view in SalesOrder
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$cur_user_id = $currentUser->getId();
		$rolesid = $currentUser->get('roleid');
		$admin_check = $currentUser->isAdminUser();
		if ($admin_check!=1 && $rolesid!='H7') {
			if ($moduleName == 'SalesOrder') {
				$listQuery .= ' AND (vtiger_crmentity.smcreatorid='.$cur_user_id.' OR vtiger_crmentity.smownerid ='.$cur_user_id.')';
			}
		}
		//custom end
		...
		foreach($listViewEntries as $recordId => $record) {
			...
			// print_r($rawData['salesorderid']);die();
			if($moduleName == 'SalesOrder'){
				 $sql = "SELECT vtiger_crmentity.modifiedby,vtiger_user2role.roleid from vtiger_crmentity INNER JOIN  vtiger_user2role ON  vtiger_user2role.userid = vtiger_crmentity.modifiedby WHERE crmid = ".$rawData['salesorderid']."";
          $result_role = $db->pquery($sql, array());
          $row = $db->fetch_row($result_role);

         
         // print_r($row);die();
			$rawData['modifiedby'] = $row['roleid'];
