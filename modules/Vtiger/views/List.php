...
class Vtiger_List_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$CustomViewLinksURL =  CustomView_Record_Model::getAllByGroup($moduleName);
    ...
    #echo "<pre>"; echo "   **  ".$this->viewName."  ->>> TEST  ".$currentUser->get('is_admin')."   ><><  ";print_r($CustomViewLinksURL);
		if(empty($this->viewName) && $currentUser->get('is_admin') != 'on'){
                       
                                        foreach($CustomViewLinksURL as $GROUP_LABEL=>$GROUP_CUSTOM_VIEWS){
                                                foreach($GROUP_CUSTOM_VIEWS as $CUSTOM_VIEW){
                                                        $customcvID = $CUSTOM_VIEW->get('cvid');
                                                        if($customcvID > 0){
                                                                break 2;
                                                        }
                                                }
               ...
               #echo $this->viewName."   ->  ";exit;
               $this->listViewModel = Vtiger_ListView_Model::getInstance($moduleName, $cvId, $listHeaders);
              # echo "<pre>"; print_r($this->listViewModel);exit;
