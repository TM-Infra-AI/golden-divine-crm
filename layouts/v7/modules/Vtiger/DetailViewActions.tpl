            ...
            <div class="pull-right btn-toolbar">
                ...
                {if $MODULE_MODEL->isStarredEnabled()}
                    {if $MODULE_NAME eq 'SalesOrder'}
                        ...
                        <button class="btn btn-default" onclick="window.location = 'index.php?module=SalesOrder&action=SalesorderBill&record={$smarty.get.record}'">{vtranslate('Export to pdf',$MODULE)}</button>
                    {/if}
                ...
                {if $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_EDIT'}
                    {if $MODULE_NAME eq 'SalesOrder'}
                    {assign var=ROLEID value=$USER_MODEL->get('roleid')}
                    {if $ROLEID eq 'H2' || $ROLEID eq 'H4' || $ROLEID eq 'H5'}
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        {foreach item=DETAIL_VIEW_LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}
                            {if $DETAIL_VIEW_LINK->getLabel() eq ""} 
                                <li class="divider"></li>   
                                {else}
                                {$delete_sales_order = vtranslate($DETAIL_VIEW_LINK->getLabel(), $MODULE_NAME)}
                                {if $delete_sales_order != "Delete Sales Estimate"}
