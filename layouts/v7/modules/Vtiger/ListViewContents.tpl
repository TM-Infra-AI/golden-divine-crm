		...
		<form name='list' id='listedit' action='' onsubmit="return false;">
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				<tbody class="overflow-y">
					...
					{assign var =  "modifyby" value=$LISTVIEW_ENTRY->rawData['modifiedby']}
					<!-- <pre> -->
					<!-- {$LISTVIEW_ENTRY->rawData['modifiedby']|@print_r} -->
						...
						{/if}
						<!-- {if ($modifyby) eq 'H4'}<tr><td>{$modifyby|@print_r}</td></tr>{/if} -->
						{if ($modifyby) eq 'H4' && $MODULE eq 'SalesOrder'}
						<tr class="listViewEntries" style="background-color: #5bc0de;" data-id='{$DATA_ID}' data-recordUrl='{$DATA_URL}&app={$SELECTED_MENU_CATEGORY}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" {if $MODULE eq 'Calendar'}data-recurring-enabled='{$LISTVIEW_ENTRY->isRecurringEnabled()}'{/if}>
