...
jQuery.Class("Vtiger_Popup_Js",{
	...
	registerEventForListViewEntryClick : function(){
		...
		popupPageContentsContainer.on('click','.listViewEntries',function(e){
			//Custom Con no. reference code GD
			var moduleName = app.getModuleName();
			var refModules = thisInstance.getModuleName();
			if (moduleName=='SalesOrder' && refModules=='Contacts') {
				console.log(this);
				console.log($(this).attr('data-info'));
				var data = $(this).attr('data-info');
				var jsonObj = $.parseJSON('[' + data + ']');
				var mobiles = jsonObj[0].mobile;
				$("#SalesOrder_editView_fieldName_cf_860").val(mobiles);
			}
