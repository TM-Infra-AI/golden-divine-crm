...
<div style='padding-top: 0;margin-bottom: 2%;padding-right:15px;'>
	...
	{if $MINILIST_WIDGET_MODEL->getTargetModule() neq 'SalesOrder'}
	...
	{if $MINILIST_WIDGET_MODEL->getTargetModule() eq 'SalesOrder'}
		<div class="customSO miniListContent" style="padding:5px">
			<table class="table">
				<tr>
					<th>Subject</th>
					{if $USER_MODEL->get('roleid') eq 'H2' || $USER_MODEL->get('roleid') eq 'H4' ||$USER_MODEL->get('roleid') eq 'H5'}
						<th>Total</th>
					{/if}
					<th>Status</th>
				</tr>
					{assign var="cmp_date" value={$smarty.now|date_format:"%Y-%m-%d"}}
			
				{foreach $CUSTOM_MINILIST as $DATA}
					{assign var = "pr_date" value = {$DATA.duedate}}
				{assign var = "st_pr" value = {$DATA.sostatus}}
				
				{if $cmp_date|strtotime > $pr_date|strtotime && $st_pr neq 'Delivered' && $pr_date neq '' && $pr_date neq NULL}
					<tr style="background-color: antiquewhite;">
						<td><a href="index.php?module=SalesOrder&view=Detail&record={$DATA.salesorderid}">{$DATA.subject}</a></td>
						{if $USER_MODEL->get('roleid') eq 'H2' || $USER_MODEL->get('roleid') eq 'H4' ||$USER_MODEL->get('roleid') eq 'H5'}
							<td><a href="index.php?module=SalesOrder&view=Detail&record={$DATA.salesorderid}">{$DATA.total|string_format:"%.2f"}</a></td>
						{/if}
						<td><a href="index.php?module=SalesOrder&view=Detail&record={$DATA.salesorderid}">{vtranslate($DATA.sostatus)}</a></td>
					</tr>
					{else}
						<tr>
						<td><a href="index.php?module=SalesOrder&view=Detail&record={$DATA.salesorderid}">{$DATA.subject}</a></td>
